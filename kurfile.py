...
class Kurfile:
	...
	###########################################################################
	# k = Kurfile(source, engine), this is doing initilization
	def __init__(self, source, engine=None):

		# logger.warning("Nice log display can work anywhere as long as logger.warning is inside a function. \n \n \t \t \t \t \t \t Right now, I am inside kurfile.Kurfile.__init__")
		...
		## Load kurfile into kurfile_object.data
		# source must be a string or a dictionary
		if isinstance(source, str):


			# give kurfile name a proper path address
			filename = os.path.expanduser(os.path.expandvars(source))

			# To test whether the path above is a real path to a real file in the directory
			if not os.path.isfile(filename):
				raise IOError('No such file found: {}. Path was expanded to: '
					'{}.'.format(source, filename))

			# set file_path to kurfile object
			self.filename = filename

			## read a kurfile.yml out and assign content to data, or spec.data
			# assign kurfile_source (a dict) to kurfile_object.data
		...
		# if source is not a string but a dictionary
		else:
			# set object.filename None
			self.filename = None
			# make sure it is a dictionary and assign to object.data
			self.data = dict(source)

		# initialize following attribute of a kurfile_object
	...
	def get_model(self, provider=None):
		...
		"""
		logger.warning("we are inside get_model")
		...
		if self.model is None:
			...
			logger.warning("Build a model using Model(backend, containers): ---")

			### Create a model using backend and containers
			self.model = Model(
				backend=self.get_backend(),
				containers=self.containers
			)

			### parse model using engine
			self.model.parse(self.engine)

			### register provider for the model
	...
	###########################################################################
	#### Create a trainer with or without optimizer
	def get_trainer(self, with_optimizer=True):
		...
		"""
		return Executor(
			### return parsed model without data provider
